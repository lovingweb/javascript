...
첫 번째 피연산자는 true 혹은 false로 평가되는 표현식을 사용할 수 있다.  
이 값이 true로 평가되면 두 번째 피연산자가 평가되고 그 값을 반환하고, false로 평가되면 세 번째 파연산자다 평가되고 그 값을 반환한다.  
...
조건에 따를 결과 값을 변수에 할당할 때도 사용할 수 있다.
...
다중 조건 if문과 같은 방식으로 여러개의 조건을 사용할 수도 있다.
...
`typeof`연산자는 피연산자의 데이터 타입 <sup>Data type</sup>을 가리키는 문자열을 반횐한다.
...
피연산자가 좌변 값이 아니거나(이 때는 연산자는 아무런 동작을 하지 않는다.) 삭제에 성공한다면 `true`를 반환, 아니면 `false`를 반환한다.

|연산자|수행되는 연산|결합방향|피연산자 개수|피연산자 타입|반환|
|:---:|---|:---:|:---:|---|---|
|`delete`|프로퍼티를 제거|R->L|1|좌변값|Boolean|
...
표현식을 평가해야 하긴 하지만 반환값이 `undefined`여야 하는 상황이 있다면 `void`연산자를 사용할 수 있겠지만, 실무에서 거의 사용되지 않는다.

다음 코드에서 `void`연산자를 사용한 이유는 바로 undefined를 얻기 위함이다. 즉 해당하는 링크가 정상적으로 동작하지 않게 만들기 위하여 이처럼 undefined를 사용한다고 보면 된다.
